...
no_completo(0, 0, _):- !, fail.
no_completo(I, 0, M):- tablero(I, 0, _), I1 is I-1, no_completo(I1, M, M).
no_completo(I, J, M):- tablero(I, J, _), J1 is J-1, no_completo(I, J1, M).
no_completo(_, _, _).
...
% Se aplico fuerza bruta, se debe verificar validez del tablero
ciclo(N, _, 1):- verifica_validez(N), write('Se verifica la validez del tablero habiendose usado fuerza bruta'), nl.
...
adfem(I, N, [X, Y|_], Color, Cambio, _):- 2 =:= Y-X, % Espacio libre a la derecha
                                          X1 is X-1,
...
adcem(J, M, [X, Y|_], Color, Cambio, _):- 2 =:= Y-X, % Espacio libre abajo
                                          X1 is X-1,
...
game(2, 7, 7):- assertz(tablero(0,2,o)),
...
game(3, 9, 9):- assertz(tablero(0,6,o)),
...
game(4, 13, 13):- assertz(tablero(0,10,x)),
...
game(5, 19, 19):- assertz(tablero(0,0,x)),
